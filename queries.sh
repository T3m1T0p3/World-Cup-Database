...
echo $($PSQL "SELECT DISTINCT name FROM teams JOIN games ON (team_id=winner_id OR team_id=opponent_id) AND round='Eighth-Final' AND year=2014 ORDER BY name ASC")

echo -e "\nList of unique winning team names in the whole data set:"
echo  $($PSQL "SELECT DISTINCT name FROM teams JOIN games ON team_id=winner_id ORDER BY name ASC")

echo -e "\nYear and team name of all the champions:"
echo $($PSQL "SELECT year,name FROM teams JOIN games ON team_id=winner_id AND round='Final' ORDER BY year ASC")
